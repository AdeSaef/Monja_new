  ...
  useEffect(() => {
    const fetchSurveyDetail = async () => {
      try {
        ...
        setDetailSurvey([surveyDetail]);  // Menyimpan data API ke state sebagai array
      } catch (error) {
        console.error("Error fetching survey data:", error);
      }
    };
    fetchSurveyDetail();
  }, [surveyId]);  // Menambahkan surveyId sebagai dependensi
  ...
  return (
    <div className="container z-50" onClick={onClose}>
      ...
      <div className="absolute top-0 h-screen w-screen min-h-full max-w-none flex flex-col py-auto z-50 overflow-hidden">
        <div>
          <h1 className="text-3xl text-white font-semibold mt-20 ml-20 z-50 cursor-pointer" onClick={onClose}>
            X
          </h1>
        </div>
        <div>
